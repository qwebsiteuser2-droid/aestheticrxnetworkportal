  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
        ...
        <h3 className="text-xl font-bold text-gray-900 text-center mb-2">Feature Not Available</h3>
        ...
        <button
          onClick={onClose}
          className="w-full py-3 px-4 bg-gradient-to-r from-blue-500 to-teal-500 text-white font-medium rounded-xl hover:opacity-95 transition-all shadow-md"
        >
          I Understand
        </button>
      </div>
...
function DoctorAvatar({
  ...
  const src = getProfileImageUrl(photoUrl);
  ...
  if (src) {
    return (
      ...
      <img
        src={src}
        alt={name}
        className={`${dim} rounded-full object-cover border-2 border-white shadow-md bg-gray-100`}
        loading="lazy"
        decoding="async"
      />
    );
  ...
  return (
    <div
      className={`${dim} rounded-full bg-gradient-to-br from-blue-500 to-teal-500 text-white flex items-center justify-center font-semibold shadow-md ${
  ...
  if (variant === 'list') {
    return (
      <>
        ...
        />
        <div className="flex items-center gap-3 p-4 bg-white border border-gray-200 rounded-xl hover:shadow-lg hover:border-blue-200 transition-all duration-200">
          <Link href={profileUrl} className="shrink-0">
            <DoctorAvatar name={doctor_name} photoUrl={profile_photo_url} size="sm" />
          </Link>
          ...
          <button
            ...
            disabled={isCreating}
            className={`ml-2 px-4 py-2 text-sm font-medium rounded-lg transition-all flex items-center gap-1.5 ${
              isCreating
    ...
    <>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden hover:shadow-lg hover:border-blue-200 transition-all duration-300">
        <div className="p-4 text-center">
          <Link href={profileUrl} className="inline-flex justify-center mb-3">
            <DoctorAvatar name={doctor_name} photoUrl={profile_photo_url} size="lg" />
          </Link>
          <Link href={profileUrl} className="block w-full">
            <h3 className="font-semibold text-gray-900 text-lg truncate hover:text-blue-600 transition-colors">
              {doctor_name}
            </h3>
          </Link>
